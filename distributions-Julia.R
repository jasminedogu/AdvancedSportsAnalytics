...
fourth_down %>%
  ...
  summarize(mean(pff_RUNPASS == "P")) # 71.43% pass, 28.57% run

# Pass
plotdist((fourth_down %>%
            filter(FIELDPOSITION <= 20,
                   pff_RUNPASS %in% c("P")
            ))$pff_GAINLOSS,
         histo = TRUE, demp = TRUE)
...
fourth_down %>%
  ...
  summarize(
    pos = mean(pff_GAINLOSS > 0 & !is.na(pff_GAINLOSS)), # 52.5%
    inc = mean(pff_GAINLOSS == 0 | is.na(pff_GAINLOSS)), # 35%
    neg = mean(pff_GAINLOSS < 0 & !is.na(pff_GAINLOSS))  # 12.5%
  )
...
denscomp(list(fw, fln, fg), legendtext = plot.legend, plotstyle = "ggplot", xlegend = "top")
fw
# Weibull(shape = 1.584, scale = 22.906)
...
fw
# -Weibull(Shape = 3.122, Scale = 6.922)
...
fourth_down %>%
  ...
  summarize(
    pos = mean(pff_GAINLOSS > 0 & !is.na(pff_GAINLOSS)), # 18.75%
    neg = mean(pff_GAINLOSS <= 0 & !is.na(pff_GAINLOSS))  # 81.25%
  )
...
# Lnorm(meanlog = 1.342, sdlog = 0.512)
...
fourth_down %>%
  ...
  summarize(mean(pff_RUNPASS == "P")) # 58.297% pass, 41.703% run
...
fourth_down %>%
  ...
  summarize(
    pos = mean(pff_GAINLOSS > 0 & !is.na(pff_GAINLOSS)), # 48.91%
    inc = mean(pff_GAINLOSS == 0 | is.na(pff_GAINLOSS)), # 43.75%
    neg = mean(pff_GAINLOSS < 0 & !is.na(pff_GAINLOSS))  # 7.33%
  )
...
# -Weibull(Shape = 1.77, Scale = 8.232)
...
fourth_down %>%
  ...
  summarize(
    pos = mean(pff_GAINLOSS > 0 & !is.na(pff_GAINLOSS)), # 71.51%
    neg = mean(pff_GAINLOSS <= 0 & !is.na(pff_GAINLOSS))  # 28.49%
  )
...
# Weibull(shape = 0.941, scale = 5.973)
...
denscomp(list(fw, fln, fg, fexp), legendtext = plot.legend, plotstyle = "ggplot", xlegend = "top")
fln
# Lnorm(meanlog = 0.847, sdlog = 0.797)
...
fourth_down %>%
  ...
  summarize(mean(pff_RUNPASS == "P")) # 46.51% pass, 53.49% run
...
fourth_down %>%
  ...
  summarize(
    pos = mean(pff_GAINLOSS > 0 & !is.na(pff_GAINLOSS)), # 45.04%
    inc = mean(pff_GAINLOSS == 0 | is.na(pff_GAINLOSS)), # 49.29%
    neg = mean(pff_GAINLOSS < 0 & !is.na(pff_GAINLOSS))  # 5.67%
  )
...
# Lnorm(meanlog = 1.197, sdlog = 0.851)
...
fourth_down %>%
  ...
  summarize(
    pos = mean(pff_GAINLOSS > 0 & !is.na(pff_GAINLOSS)), # 69.7%
    neg = mean(pff_GAINLOSS <= 0 & !is.na(pff_GAINLOSS))  # 30.296%
  )
...
denscomp(list(fw, fln, fg, fexp), legendtext = plot.legend, plotstyle = "ggplot", xlegend = "top")
fln
# Lnorm(meanlog = 0.625, sdlog = 0.572)
...
glm(data = data, pff_DEEPPASS ~ pff_DOWN + pff_DISTANCE + FIELDPOSITION) -> model
summary(model)